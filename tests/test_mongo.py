...
def test_demercado_free_add_then_drop_leaves_unknown_without_faab_change(session):
    # Sep 11 free add remains in history; Sep 17 drop ends current ownership.
    ...
    week1 = session.scalar(select(LineupAssignment).where(
        LineupAssignment.league_id == lg.id,
        LineupAssignment.player_id == demercado.id,
        LineupAssignment.season == 2026,
        LineupAssignment.week == 1,
    ))
    assert week1.slot == "BN"
    assert week1.placement == "BENCH"
    add = session.scalar(select(TransactionEvent).where(
        ...
        TransactionEvent.group_id == "demercado-add-20260911-free-001",
    ))
